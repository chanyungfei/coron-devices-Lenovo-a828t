...
#        if [ -n "$isInstalled" -o ! -e "$DATA_HASH_DIR/$file.flag" ]; then
            rm /data/local/tmp/preinstall.log
            busybox sh /system/bin/pm install -r $PRELOAD_APP_DIR/$file >> /data/local/tmp/preinstall.log
            ret=$?
            if [ $ret -ne 0 ]; then
                echo "$file: install failed, error: $ret"
#               echo "$file: install failed, error: $ret" >> $PRELOAD_LOG_FILE
            else
                preinstall_ret=`cat /data/local/tmp/preinstall.log`
                echo "$file: @@@@@@@@@@@@@ $preinstall_ret"
                if [ "$preinstall_ret" == "Success" ]; then
...
#                   echo "$file: install successful, copying $file.flag to $DATA_HASH_DIR" >> $PRELOAD_LOG_FILE
                   ...
                   preinstall_ret=`cat /data/local/tmp/preinstall.log`
                   if [ "$preinstall_ret" == "Success" ]; then
                       echo "$file: install successful,retry 1, copying $file.flag to $DATA_HASH_DIR"
                       busybox cp $PRELOAD_HASH_DIR/$file.flag $DATA_HASH_DIR
                   else
                       rm /data/local/tmp/preinstall.log
                       busybox sh /system/bin/pm install -r $PRELOAD_APP_DIR/$file >> /data/local/tmp/preinstall.log
                       preinstall_ret=`cat /data/local/tmp/preinstall.log`
                       if [ "$preinstall_ret" == "Success" ]; then
                          echo "$file: install successful, retry 2, copying $file.flag to $DATA_HASH_DIR"
                          busybox cp $PRELOAD_HASH_DIR/$file.flag $DATA_HASH_DIR
                       else
                          echo "$file: $preinstall_ret install failed"
                       fi
                   fi    
                fi
            fi
